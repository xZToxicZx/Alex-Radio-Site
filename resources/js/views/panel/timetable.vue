            ...
            <tbody>
                <tr v-for="(days, hr) in slots">
                    ...
                    <td v-for="slot in days">
                        ...
                        <button type="button" class="btn btn-danger btn-sm" v-else-if="slot.user != null && slot.user.id == $auth.user().id && ((slot.time - 14400) * 1000) > new Date().getTime() && canUnclaimSlot" @click="claim(slot.time, true)">Unclaim</button>
